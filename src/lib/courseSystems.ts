 ...
 * namn. De är lästa ur två anordnares egna tabeller, som båda skriver ut de två
 * systemen på samma rad. Namnen är datans egen stavning av respektive kod, så
 * en listning och dess motsvarighet aldrig kan säga olika saker om samma kurs.
 *
 * Kurser som bara finns i ett av systemen står inte här, och inte heller de
 * rader där överföringen inte är en till en: Datorteknik 1a och 1b blir en enda
 * ämnesnivå, Matematik specialisering blir två. Ett par ska gå att läsa åt båda
 * hållen, annars säger appen hellre ingenting.
 *
 * Tabellerna säger samma sak där de överlappar — trettiosex par fanns i båda,
 * utan en enda motsägelse — vilket är skälet att våga lita på de fyrtionio som
 * bara den ena har.
 *
 * Källor:
 * - gymnasieskolor.orebro.se, Komvux Örebros prövningstabell hösten 2026,
 *   läst 2026-09-10.
 * - media.helsingborg.se, "Gymnasiala kurser distans — Gamla kurser / Nya
 *   kurser Gy25", Helsingborgs stads jämförelselista för betygsprövning,
 *   läst 2026-09-15. Tio rader ur den står inte här, alla för att kurskoden är
 *   feltryckt: Psykologi nivå 2 bär Psykologi nivå 1:s kod, Specialpedagogik 1
 *   har ett S för mycket, och spanskan, italienskan och danskan saknar nollan
 *   som franskans och tyskans moderna språk-koder har på raderna ovanför. En
 *   felstavad kod går att se — att rätta den är att gissa.
 */
...
export const COURSE_PAIRS: CoursePair[] = [
  ...
  { gy11: c('HALAHL0', 'Hälsopedagogik'), gy25: c('HALS1000X', 'Hälsopedagogik Nivå 1') },
  {
