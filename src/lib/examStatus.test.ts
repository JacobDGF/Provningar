...
describe('isOpenForRegistration', () => {
  it('is open inside a full window', () => {
    ...
    expect(isOpenForRegistration(examWith({
      label: '', applicationStart: '2026-07-27', applicationEnd: '2026-08-23', confirmed: true,
    }))).toBe(true);
  });
  ...
  it('treats a deadline with no stated opening as open', () => {
    at('2026-08-09T12:00:00Z');
    expect(isOpenForRegistration(examWith({
      label: '', applicationEnd: '2026-08-11', confirmed: true,
    }))).toBe(true);
  });
  ...
  it('never reports an unconfirmed period as open', () => {
    at('2026-08-09T12:00:00Z');
    expect(isOpenForRegistration(examWith({
      label: '', applicationStart: '2026-07-27', applicationEnd: '2026-08-23', confirmed: false,
    }))).toBe(false);
  });
...
describe('open-ended application windows', () => {
  it('is open once an opening date has passed and no closing date is published', () => {
    at('2026-08-09T12:00:00Z');
    expect(isOpenForRegistration(examWith({
      label: '', applicationStart: '2026-07-01', confirmed: true,
    }))).toBe(true);
  });
