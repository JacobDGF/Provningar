...
const FAQ: FaqItem[] = [
  ...
  },
  {
    ...
    a: (
      <>
        Avgiften är lagstadgad och kostar <strong>500 kr</strong> per prövning hos alla anordnare, enligt
        Förordning (1991:1124) om avgifter för prövning inom skolväsendet. Har du sedan tidigare betyget{' '}
        <strong>F</strong> i kursen är prövningen <strong>kostnadsfri</strong>.
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    a: (
      <>
        Gymnasiekurser. Observera att de gamla kurskoderna (Gy11, t.ex. <em>MATMAT02b</em>) gäller för prövning
        till och med <strong>30 juni 2030</strong>. Därefter gäller de nya Gy25-kurserna. Vi uppdaterar
        uppgifterna löpande.
      </>
    ),
  },
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end" onClick={onClose}>
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 p-4 space-y-2.5">
          {FAQ.map((item, i) => (
            <FaqRow key={i} item={item} open={openIdx === i} onToggle={() => setOpenIdx(openIdx === i ? null : i)} />
          ))}
