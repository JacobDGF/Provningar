...
import { Search, SlidersHorizontal, X, TrendingUp, Navigation, ShieldCheck, HelpCircle } from 'lucide-react';
      ...
      <div className="bg-surface px-4 pt-14 pb-4 sticky top-0 z-30 border-b border-line">
        <div className="flex items-start justify-between mb-3">
          ...
          <button
            ...
            className="w-10 h-10 bg-sand rounded-2xl flex items-center justify-center flex-shrink-0 active:scale-90"
          >
        ...
        <div className="flex gap-2">
          <div className="flex-1 flex items-center gap-2 bg-sand rounded-2xl px-3 py-3">
          ...
          <button
            ...
            className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${
              hasActiveFilters ? 'bg-brand-500 text-white' : 'bg-sand text-ink-soft'
            }`}
          >
          ...
          <div className="flex gap-2 mt-3 overflow-x-auto pb-1 scrollbar-hide">
            {FEATURED_SUBJECTS.map(s => (
              <button
                ...
                className="flex-shrink-0 bg-brand-50 text-brand-700 text-xs font-semibold px-3 py-1.5 rounded-full"
              >
                {s}
              </button>
            ))}
          ...
          <div className="px-4 py-4 space-y-4">
            ...
            {!userLocation && locationStatus !== 'denied' && (
              <button
                ...
                className="w-full flex items-center gap-3 bg-brand-50 border border-brand-100 rounded-2xl p-4 text-left active:scale-98 transition-transform"
              >
